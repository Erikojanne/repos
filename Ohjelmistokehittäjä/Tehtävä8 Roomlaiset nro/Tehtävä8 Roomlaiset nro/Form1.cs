...
namespace Tehtävä8_Roomlaiset_nro_
{
    public partial class RoomalaisiksiForm : Form
    {
        ...
        private void MuutaBT_Click(object sender, EventArgs e)
        {
            ...
            else if (TekstiTB.Text.Length > 2) // kyseessä sataluku
            {
                luku2 = Convert.ToInt32(TekstiTB.Text.Substring(0, 1)); // Sadat
                luku3 = Convert.ToInt32(TekstiTB.Text.Substring(1, 1)); // Kymmenet
                luku4 = Convert.ToInt32(TekstiTB.Text.Substring(2, 1)); // ykköset
                vastaus += sataset(luku2, vastaus);
                vastaus += kympit(luku3, vastaus);
                vastaus += ykkoset(luku4, vastaus);
            }
            else if (TekstiTB.Text.Length > 1) // kyseessä kymmenluku
            {
                luku3 = Convert.ToInt32(TekstiTB.Text.Substring(0, 1)); // Kymmenet
                luku4 = Convert.ToInt32(TekstiTB.Text.Substring(1, 1)); // ykköset
                vastaus += kympit(luku3, vastaus);
                vastaus += ykkoset(luku4, vastaus);
            }
            else if (TekstiTB.Text.Length > 0) // kyseessä ykköset
            {
        ...
        private string sataset(int luku2, string vastaus) // koska palauttaa arvon niin private string
        {
